...
struct MapView: UIViewRepresentable {
    ...
    func updateUIView(_ mapView: MKMapView, context: Context) {
        ...
        if workoutsManager.finishedLoading {
            if let selectedWorkout = workoutsManager.selectedWorkout {
                mapView.addOverlay(MKGeodesicPolyline(coordinates: selectedWorkout.routeLocations.map { $0.coordinate}, count: selectedWorkout.routeLocations.count))
            }
        }
    }
}
